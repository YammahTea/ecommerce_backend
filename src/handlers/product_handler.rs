...
pub async fn update_product(State(state): State<AppState>, Path(product_id): Path<Uuid>, Json(user_product_payload): Json<UpdateProductRequest>) -> Result<impl IntoResponse, UpdateProductError> {
    let updated_product = edit_product(product_id, &user_product_payload, &state.pool).await?;

    Ok((StatusCode::OK, Json(updated_product)))
}

pub async fn delete_product(State(state): State<AppState>, Path(product_id): Path<Uuid>) -> Result<impl IntoResponse, SoftDeleteProductError> {
